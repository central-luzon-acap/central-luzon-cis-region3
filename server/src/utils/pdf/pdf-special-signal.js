 ...
 * @param {String} recommendationsPractices - Management practices recommendations as formatted HTML tags. If supplied, make `recommendationsByStage=null`
 ...
 */
const specialSignalTemplatePDF = async (
  location,
  language,
  cycloneData,
  windspeedData,
  ...
  try {
    ...
    const fallbackImage = path.join(__dirname, 'livereload', 'public', 'special-signal', 'images', 'track_maymay_lowres.png')
  ...
  for (let i = 0; i < specialBulletinImages.length; i += 1) {
    images.push(path.resolve(__dirname, 'livereload', 'public', 'special-signal', 'images', specialBulletinImages[i]))
  }

  // Load the HTML file
  html = template(path.resolve(__dirname, 'templates', 'pdf-special-signal.ejs'), images, '../livereload/public/special-signal/images')

  // Replace the cyclone image with the latest typhoon picture
  if (cycloneImage) {
    const prefix = 'data:image/png;base64,'
    html = html.replace('../livereload/public/special-signal/images/track_maymay_lowres.png', `${prefix}${cycloneImage}`)
  }
  ...
  const js = [path.resolve(__dirname, 'livereload', 'public', 'special-signal', 'main.js')]

  for (let i = 0; i <= 3; i += 1) {
    css.push(path.resolve(__dirname, 'livereload', 'public', 'special-signal', `section-0${i}.css`))
  }
...
module.exports = specialSignalTemplatePDF
