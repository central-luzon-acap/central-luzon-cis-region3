...
module.exports.updateUser = async (req, res, next) => {
  ...
  const { account_level } = req.user
  let userinfo = {}

  switch (account_level) {
    case ACCOUNT_LEVEL.SUPERADMIN:
      userinfo = { ...req.body }
      break
    case ACCOUNT_LEVEL.ADMIN:
      if (uid !== req.user.uid) {
        return res.status(500).send('You cannot update other user\'s password.')
      } else {
        userinfo = { uid, password: req.body.password }
      }
      break
    default:
      break
