 ...
 */
const createNewSeasonalReport = async ({
  REGION_LOCATIONS,
  region,
  province,
  month,
      ...
      if (!isFull) {
        ...
        if (!validmo) {
          ...
          if (condition === NO_DATA_AVAILABLE) {
            const eMessage = `(${province}) No rainfall condition data available for the selected month of "${month}"`
  ...
  //    */
  //   // TODO: sanityChecker(recommendations, language)
  // } catch (err) {
  //   throw new Error(err)
  // }
  ...
  try {
    ...
    if (activities.length && stages.length) {
      recommendations.group = grouprecommendationsV2(
        recommendations,
        activities,
        stages
      )
    ...
    if (activities.length && stages.length) {
      ...
      content +=
        recommendations[0][
          `management_recommendations_${language_full[language]}`
        ]
    }
    ...
    if (activities.length && stages.length) {
      ...
      content += recommendations[0][`impact_outlook_${language_full[language]}`]
    }
  } catch (err) {
    throw new Error(err.message)
  }

  const recsTitle = `${province} - ${crop}`
  ...
  let page
  let id
  let filename = `${province}_${crop}_${monthFileName}_${year}`
  ...
  try {
    ...
    let _hasProvincial
    if (Array.isArray(cropping_calendar) && cropping_calendar.length === 0) {
      // The variable is an empty array
      _hasProvincial = false
    } else if (
      typeof cropping_calendar === 'object' &&
      cropping_calendar !== null &&
      !Array.isArray(cropping_calendar)
    ) {
      // The variable is an object with values
      _hasProvincial = true
    }
    ...
    const { html, css, js } = await seasonalTemplatePDF({
      ...
      recommendationsImpacts:
        activities.length && stages.length
          ? language === 'en'
            ? impactsContent.impact
            : impactsContent.impact_tagalog
          : recommendations[0][`impact_outlook_${language_full[language]}`],
      mainTitle: recsTitle,
      hasProvincial: _hasProvincial,
  ...
  if (operation === 'create') {
    try {
      ...
      const monthsRange = `${capitalizeFirstLetter(
        fdata.mos[0]
      )} - ${capitalizeFirstLetter(fdata.mos[fdata.mos.length - 1])}`
      ...
      const response = await createreport({
        ...
        stages: stages ? stages.map((stage) => stage.label) : [],
        activities,
        risk: climateRisk.label,
    ...
    return new Promise((resolve, reject) => {
      ...
      const pdfRef = bucket.file(
        `${FIRESTORE_COLLECTIONS.PDF_STORAGE_SEASONAL}/${filename}.pdf`
      )
...
module.exports = createNewSeasonalReport
