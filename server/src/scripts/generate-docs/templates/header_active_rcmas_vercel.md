...
> - You are viewing an alternate 1:1 version of the official ACAP {{REGION_NAME}} hosted on Render ([render.com](https://render.com/)).
> - These APIs, hosted in [Vercel](https://vercel.com/), aim to provide _"alternate access"_ and _"faster ACAP-RCMAS API response times"_ from slow server cold starts since the official ACAP {{REGION_NAME}} backend still uses a standard Render pricing plan.
> - To note, Render server apps running on a standard pricing plan have a 1 - 3 minute cold start after 15 minutes of inactivity, while the standard plan Vercel server apps only have about 20 - 30 seconds cold start.
> - The Render and Vercel-hosted versions, sharing the same code base and data sources, have their data regularly updated as mentioned in the documentation.
> - Please follow this [[link]]({{http://RENDER_API_URL}}/docs) should you want to use and access the official Render-hosted version of these APIs.
...
Collaborators with access to the ACAP-RCMAS APIs can alternately access it here at:
- [{{http://API_URL}}]({{http://API_URL}})
