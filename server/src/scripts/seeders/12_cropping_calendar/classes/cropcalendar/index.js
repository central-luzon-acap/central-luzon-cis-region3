...
const { CROP_STAGE_LABELS } = require('../../lib/constants')

class CroppingCalendar extends CsvToFireStore {
  constructor ({ csvFilePath, municipalitiesToReplace }) {
    super(csvFilePath)
    ...
    // this.missingMunicipalities = MUNICIPALITIES_TO_REPLACE_WITH_PAGASA.map(item => item.municipality)
    this.missingMunicipalities = municipalitiesToReplace
      ? municipalitiesToReplace.map(item => item.municipality)
      : []

    this.replaceMissingMunicipalities = municipalitiesToReplace
  }
    ...
    headers.forEach(item => {
      ...
      if (key === 'municipality' && value !== '') {
        ...
        if (this.missingMunicipalities.length > 0) {
          if (this.missingMunicipalities.includes(tempMunicipality)) {
            tempMunicipality = this.replaceMissingMunicipalities.find(x =>
  ...
  groupByProvince (data) {
    return (data || this.csv_rows).reduce((group, row, index) => {
