...
const validateExcel = require('./validate')
const { DOWNLOAD_DIR } = require('../lib/constants')
...
const downloadValidateExcel = async (params) => {
  const { url, storagePath, dest = 'sample.xlsx', ExcelDefinition } = params
  const { tocsv = false, deleteExcel = true, dayNum = 0 } = params
    ...
    https.get(url, (res) => {
      ...
      file.on('finish', () => {
        file.close(async () => {
          try {
            let extracted = []
            extracted = await validateExcel({
              REGION_LOCATIONS: params?.REGION_LOCATIONS ?? null,
              excelFile: filePath,
              ExcelDefinition,
              dayNum,
              tocsv
