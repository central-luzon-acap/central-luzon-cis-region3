...
const ASSET_KEY = 'og_superadmin'

// NextJS static props
export async function getStaticProps () {
  let media = { description: '', url: '', path: `${process.env.BASE_URL}/superadmin/login` }
  ...
  return (
    <>
      {(process.env.BASE_URL === process.env.BASE_URL_PROD) &&
        <ShareHead
          title={`Agro-Climatic Advisory Portal - ${REGION_NAME} (ACAP-${REGION_NAME.toUpperCase()}) Superadmin Login`}
